...
public class EncasedVoidFilterBlock extends Block implements Shootable {
	...
	public boolean onShotByRifle(World world, BlockState bs, LivingEntity user, RifleMode mode, float power, BlockPos pos, BlockHitResult bhr) {
		if (mode == RifleMode.EXPLODE && power > 1.1f && world.getRegistryKey().getValue().toString().equals("minecraft:overworld") && pos.getY() == world.getBottomY()+2 && bhr.getSide() == Direction.UP) {
			...
			if (world.getBlockState(down).isOf(YBlocks.BEDROCK_SMASHER) && world.getBlockState(downDown).isOf(Blocks.BEDROCK)) {
				...
				user.getWorld().createExplosion(null, user.getDamageSources().explosion(user, user), null, bhr.getPos().x, bhr.getPos().y, bhr.getPos().z, 3.3f, false, World.ExplosionSourceType.NONE);
				return true;
			}
		}
		return false;
