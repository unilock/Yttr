...
public class SpectralAxeItem extends AxeItem implements TicksAlwaysItem, ItemColorProvider {
	...
	public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
		...
		if (!(entity instanceof PlayerEntity) && entity.age > 0) {
			playGlobalVanishEffect(entity.getWorld(), entity.getPos().x, entity.getBoundingBox().getCenter().y, entity.getPos().z);
			stack.setCount(0);
			return;
		}
		if (!stack.hasNbt()) {
	...
	private void playVanishEffect(Entity e) {
		if (e instanceof ServerPlayerEntity) {
			...
			PlaySoundFromEntityS2CPacket spkt = new PlaySoundFromEntityS2CPacket(YSounds.SPECTRAL_AXE_DISAPPEAR_HOLDER, SoundCategory.PLAYERS, e, 1f, 1, 7);
			Vec3d hand = RifleItem.getMuzzlePos(e, true);
			ParticleS2CPacket ppkt = new ParticleS2CPacket(new DustParticleEffect(new Vector3f(0.95f, 0.95f, 1), 0.6f), false, hand.x, hand.y, hand.z, 0.25f, 0.125f, 0.25f, 0, 20);
	...
	private void playGlobalVanishEffect(World w, double x, double y, double z) {
		...
		if (w instanceof ServerWorld) {
			((ServerWorld)w).spawnParticles(new DustParticleEffect(new Vector3f(0.95f, 0.95f, 1), 0.6f), x, y, z, 20, 0.5f, 0.5f, 0.5f, 0);
		}
	}
