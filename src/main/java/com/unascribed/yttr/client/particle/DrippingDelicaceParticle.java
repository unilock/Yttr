...
public class DrippingDelicaceParticle extends BlockLeakParticle {
	...
	public DrippingDelicaceParticle(ClientWorld clientWorld, double x, double y, double z) {
		...
		setColorAlpha(0.5f);
		SpriteProvider sprites = (SpriteProvider) ((AccessorParticleManager)MinecraftClient.getInstance().particleManager).yttr$getSpriteAwareFactories().get(Registries.PARTICLE_TYPE.getKey(ParticleTypes.DRIPPING_HONEY).get().getValue());
		setSprite(sprites);
	}
