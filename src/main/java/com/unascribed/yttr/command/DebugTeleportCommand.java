...
public class DebugTeleportCommand {
	...
	public static LiteralArgumentBuilder<ServerCommandSource> create(CommandBuildContext bctx) {
		return CommandManager.literal("yttr:dtp")
				...
				.then(CommandManager.argument("state", BlockStateArgumentType.blockState(bctx))
					.executes((ctx) -> {
						...
						if (idx >= 0) {
							...
							ctx.getSource().sendFeedback(Text.translatable("commands.teleport.success.entity.single", p.getDisplayName(), stateStr), true);
						}
						return 0;
					})
				);
